...
* Version 2.3.4 [2021-01-03 Sun]
  - Modified minimize/maximize functions in "sugar" script for glucose output
  - Bug fix of "maximize" routine in sugar Perl script.
  - Support for multiple solutions by a SAT solver.
    For example, 'sugar -solver "picosat --all' xxx.csp' shows all CSP solutions.
    However, there might be some duplications in the shown CSP solutions.
  - Expressions of the form "(ignore ...)" are ignored at parsing.
